...
#include "ths_blurfilter_vs20.inc"
#include "ths_blurfilter_ps20.inc"
#include "ths_blurfilter_ps20b.inc"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

BEGIN_VS_SHADER_FLAGS( THS_BlurFilterY, "Help for THS_BlurFilterY", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( ths_blurfilter_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( KERNEL, params[ KERNEL ]->GetIntValue() ? 1 : 0 );
			SET_STATIC_VERTEX_SHADER( ths_blurfilter_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( ths_blurfilter_ps20b );
				...
				SET_STATIC_PIXEL_SHADER( ths_blurfilter_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( ths_blurfilter_ps20 );
				SET_STATIC_PIXEL_SHADER_COMBO( KERNEL, params[ KERNEL ]->GetIntValue() );
				SET_STATIC_PIXEL_SHADER_COMBO( CLEAR_COLOR, params[ ENABLECLEARCOLOR ]->GetIntValue() );
				SET_STATIC_PIXEL_SHADER( ths_blurfilter_ps20 );
			}
...
//			dY *= 0.4;
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( ths_blurfilter_ps20 );
			SET_DYNAMIC_VERTEX_SHADER( ths_blurfilter_ps20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( ths_blurfilter_ps20b );
				SET_DYNAMIC_PIXEL_SHADER( ths_blurfilter_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( ths_blurfilter_ps20 );
				SET_DYNAMIC_PIXEL_SHADER( ths_blurfilter_ps20 );
			}
		}
		Draw();
	}
