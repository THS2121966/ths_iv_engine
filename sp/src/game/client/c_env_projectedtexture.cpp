...
static ConVar thsdev_light_volumetrics_fade_range( "thsdev_light_volumetrics_fade_range", "128.0", FCVAR_CHEAT  );
ConVar thsdev_light_volumetrics_enabled( "thsdev_light_volumetrics_enabled", "1", FCVAR_ARCHIVE );

void thsdev_volumetrics_subdiv_Callback( IConVar *var, const char *pOldValue, float flOldValue )
{
...
static ConVar thsdev_light_volumetrics_subdiv( "thsdev_light_volumetrics_subdiv", "0", 0, "", thsdev_volumetrics_subdiv_Callback );
...
int C_EnvProjectedTexture::DrawModel( int flags )
{
	if ( !m_bState ||
		...
		!thsdev_light_volumetrics_enabled.GetBool() ||
	...
	if ( m_flVolumetricsFadeDistance > 0.0f )
	{
		...
		flDistanceFade = RemapValClamped( delta.Length(),
			m_flVolumetricsFadeDistance + thsdev_light_volumetrics_fade_range.GetFloat(),
...
void C_EnvProjectedTexture::RebuildVolumetricMesh()
{
	...
	const int iCvarSubDiv = thsdev_light_volumetrics_subdiv.GetInt();
